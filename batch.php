...
<body class="bg-white text-gray-800">
    ...
    <main class="py-12 overflow-hidden">
        <section class="py-8">
            <div
            ...
            <div class="container mx-auto">
                <div id="course-batch-container"
                    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center">
                <!-- Course will be dynamically loaded here -->
        ...
        <section class="py-12 px-8 bg-gradient-to-r from-purple-100 to-gray-200">
            <h2 class="text-4xl font-semibold mb-2 text-center text-rose-600">Demo Classes</h2>
            <p class="text-lg text-center text-black mb-8">( Grasp concepts with our expert instructors )</p>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-6" id="demo-class">
                <div class="aspect-video">
                    <iframe src="https://www.youtube.com/embed/ysz5S6PUM-U" title="Demo 1" frameborder="0"
                        allowfullscreen class="w-full h-full rounded-lg"></iframe>
                </div>
                <div class="aspect-video">
                    <iframe src="https://www.youtube.com/embed/J---aiyznGQ" title="Demo 2" frameborder="0"
                        allowfullscreen class="w-full h-full rounded-lg"></iframe>
                </div>
                <div class="aspect-video">
                    <iframe src="https://www.youtube.com/embed/oHg5SJYRHA0" title="Demo 3" frameborder="0"
                        allowfullscreen class="w-full h-full rounded-lg"></iframe>
                </div>
                <div class="aspect-video">
                    <iframe src="https://www.youtube.com/embed/lTTajzrSkCw" title="Demo 4" frameborder="0"
                        allowfullscreen class="w-full h-full rounded-lg"></iframe>
                </div>
            </div>
        ...
        <section class="relative py-10 px-8 overflow-x-hidden bg-white">
            <h2 class="text-4xl font-semibold mb-6 text-center text-indigo-600">Frequently Asked Questions</h2>
            <!-- Horizontal animated typography (ticker) -->
            <div class="relative flex items-center justify-center mb-8 min-h-[58px]">
                <div id="faq-ticker-pin" class="w-full max-w-2xl mx-auto">
                    <div id="faq-ticker-track"
                        class="flex justify-center whitespace-nowrap text-2xl font-semibold tracking-wide text-purple-700 gap-16">
                        <!-- These text blocks will be "typed" in one by one with animation -->
                        <span class="faq-ticker-item">📅 Course Duration: 12 months</span>
                        <span class="faq-ticker-item">🎥 Classes: Hybrid Mode (Live & Recorded)</span>
                        <span class="faq-ticker-item">📚 Practice Material Provided</span>
                    </div>
                </div>
            </div>
            <!-- FAQ content grid -->
            <div class="flex flex-col md:flex-row gap-6 max-w-7xl mx-auto ">
                <div
                    class="faq-card p-5 rounded-xl shadow-xl border-2 border-purple-200/60 hover:border-purple-400 transition-all duration-300 bg-gradient-to-r from-white via-purple-50 to-blue-50">
                    <h3 class="font-semibold text-xl mb-4">What is the duration of the course?</h3>
                    <p>
                        📅 The course duration is <span class="font-semibold text-purple-700">12 months</span>, with
                        engaging
                        classes throughout the year for full mastery of the syllabus.
                    </p>
                </div>
                <div
                    class="faq-card p-5 rounded-xl shadow-xl border-2 border-blue-200/60 hover:border-blue-400 transition-all duration-300 bg-gradient-to-r from-white via-blue-50 to-teal-50">
                    <h3 class="font-semibold text-xl mb-4">Are the classes recorded or live?</h3>
                    <p>
                        🎥 All classes are conducted in <span class="font-semibold text-blue-700">hybrid mode</span>,
                        meaning you
                        get both <b>live interactive sessions</b> and full <b>recorded access</b>—so you can learn at
                        your own pace!
                    </p>
                </div>
                <div
                    class="faq-card p-5 rounded-xl shadow-xl border-2 border-green-200/60 hover:border-green-400 transition-all duration-300 bg-gradient-to-r from-white via-green-50 to-lime-50">
                    <h3 class="font-semibold text-xl mb-4">What study/practice materials are provided?</h3>
                    <p>
                        📚 We provide comprehensive <span class="font-semibold text-green-700">practice
                            materials</span>,
                        including detailed notes, up-to-date question banks, and previous year papers—all tailored for
                        top
                        competitive success.
                    </p>
                </div>
                <div
                    class="faq-card p-5 rounded-xl shadow-xl border-2 border-pink-200/60 hover:border-pink-400 transition-all duration-300 bg-gradient-to-r from-white via-pink-50 to-purple-50">
                    <h3 class="font-semibold text-xl mb-4">How are students evaluated during the course?</h3>
                    <p>
                        📝 We conduct <span class="font-semibold text-blue-700">regular class tests</span> to ensure
                        consistent
                        progress
                        and understanding of concepts.
                        In addition, <span class="font-semibold text-purple-700">monthly mock tests</span> are organized
                        to simulate
                        real exam conditions, helping students assess their preparedness and adapt their strategies for
                        competitive
                        exams.
                    </p>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Get track and level params from URL
            ...
            if (track && level) {
                // 1. Fetch course info
                fetch(`php_backend/api/courses.php?track=${encodeURIComponent(track)}&level=${encodeURIComponent(level)}`)
                    .then(r => r.json())
                    .then(data => {
                        if (!data || !Array.isArray(data) || !data.length) {
                            document.getElementById('course-batch-container').innerHTML = "<p>No courses found.</p>";
                            return;
                        }
                        // Only use the first course for this batch (or render all if you want)
                        renderCourseBatches(data);
                    });

                // 2. Fetch demo videos (assume similar pattern with API)
                fetch(`php_backend/api/demo_class_vid.php?track=${encodeURIComponent(track)}&level=${encodeURIComponent(level)}`)
                    .then(r => r.json())
                    .then(showDemoVideos)
                    .catch(() => { showDemoVideos([]); });
            }

            function renderCourseBatches(courses) {
                const container = document.getElementById('course-batch-container');
                container.innerHTML = courses.map(course => `
    <div id="cohort-card"
      class="bg-white w-80 shadow rounded-lg p-4 flex flex-col border border-amber-700 transition-transform duration-200 ease-in-out hover:shadow-2xl hover:scale-105">
      <img src="${course.batch_image_path || 'fallback.jpg'}"
        alt="${course.title || ''}" class="w-96 h-full object-cover rounded-md mb-4">
  ...
  `).join('');
            ...
            function showDemoVideos(response) {
                console.log('Demo videos response:', response);
                const demoClass = document.getElementById('demo-class');
                demoClass.innerHTML = '';

                // Handle the response structure from the API
                let videos = [];
                if (response && response.demo_class_videos) {
                    videos = response.demo_class_videos;
                ...
                if (!Array.isArray(videos) || !videos.length) {
                    demoClass.innerHTML = '<p class="text-gray-700 col-span-4">No demo videos found.</p>';
                    return;
                }

                videos.forEach((v, index) => {
                    console.log(`Processing video ${index}:`, v);
                    const ytID = extractYouTubeID(v.video_link || v.youtube_url || v.link || v.url);
                    console.log(`Extracted YouTube ID: ${ytID}`);

                    if (ytID) {
                        demoClass.innerHTML += `
          <div class="aspect-video">
            <iframe src="https://www.youtube.com/embed/${ytID}" title="Demo ${index + 1}" frameborder="0" allowfullscreen
            ...
            // Utility to extract YouTube ID from full link
